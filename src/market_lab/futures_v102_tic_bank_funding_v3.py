"""V102 supports headings outside the single TIC table; economics unchanged."""
...
from market_lab import futures_v102_tic_bank_funding_v2 as predecessor

original = predecessor.original

base, engine, STORAGE = original.base, original.engine, original.STORAGE
CONFIG = base.PROJECT / "configs/v102_tic_bank_funding_v3.json"
SEAL = base.PROJECT / "configs/v102_tic_bank_funding_v3.seal.json"
...
def parse_release(raw, date, url):
    ...
    parser.feed(decoded)
    inside = [t for t in parser.tables if any(original.TITLE in row for row in t)]
    outside = not inside
    if outside:
        unit = "(Billions of dollars, not seasonally adjusted)"
        text_parser = original.prior.prior.TextParser()
        text_parser.feed(decoded)
        text = original.normalize(" ".join(text_parser.parts))
        base.require(len(parser.tables) == 1 and parser.rows is None
                     and text.count(original.TITLE) == 1 and text.count(unit) == 1,
                     "ambiguous outside table headings")
        base.require(sum(bool(r and r[0] == "29") for r in parser.tables[0]) == 1,
                     "outside heading has no unique bank row")
        # Derived parser memory only: preserve all original cells/bytes on disk.
        insertion = (f"<tr><td>{original.TITLE}</td></tr>"
                     f"<tr><td>{unit}</td></tr>")
        decoded, count = re.subn(r"(<table\b[^>]*>)", lambda m: m[1] + insertion,
                                 decoded, flags=re.I)
        base.require(count == 1, "ambiguous table insertion")
        raw = decoded.encode("utf-8")
    row = predecessor.parse_release(raw, date, url)
    return {**row, "headings_outside_table": outside}
...
def collect(cfg, expected):
    ...
    base.require(failure["error"] == "ambiguous TIC table"
                 and failure["parsed_releases"] == 24 and failure["new_http_requests"] == 6,
    ...
    try:
        ...
        base.require(requests == 68 and reused == 28, "unexpected request/reuse count")
...
def verify_source(cfg, expected, manifest_sha):
    ...
    base.require(manifest["seal_sha256"] == expected and manifest["new_http_requests"] == 68
                 and manifest["reused_releases"] == 28, "source seal/count drift")
    dates = original.select_dates((root / "raw/index.html").read_bytes(), cfg)
...
def load(expected):
    base.require(base.sha(SEAL) == expected, "V102 V3 seal drift")
    for name, digest in original.read_json(SEAL)["files"].items():
        base.require(base.sha(base.safe(base.PROJECT, name)) == digest, "V102 V3 file drift")
    _, parent = predecessor.load(original.read_json(CONFIG)["parent_v102_v2_seal_sha256"])
