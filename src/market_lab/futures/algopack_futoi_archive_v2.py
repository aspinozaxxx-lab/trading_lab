...
PROTOCOL = "algopack_futoi_archive_v2"
...
def verify(seal_sha: str) -> dict:
    ...
    required = {f"configs/{PROTOCOL}.json", f"src/market_lab/futures/{PROTOCOL}.py",
                f"tests/test_{PROTOCOL}.py", "docs/ALGOPACK_FUTOI_ARCHIVE_V2.md",
...
def parse(raw: bytes, day: str, ticker: str | None = None) -> tuple[dict, dict]:
    ...
    try:
        ...
        latest, counts, unpaired = {}, {}, {}
        for name, points in groups.items():
            by_group: dict[str, list[tuple[tuple, dict]]] = {}
            for point, records in points.items():
                for row in records:
                    by_group.setdefault(row["clgroup"], []).append((point, row))
            if ticker is None and any(len(items) != 1 for items in by_group.values()):
                raise Error("daily_latest_group_not_unique")
            latest[name] = [max(items, key=lambda item: item[0])[1]
                            for items in by_group.values()]
            counts[name] = sum(len(records) == 2 for records in points.values())
            unpaired[name] = sum(len(records) != 2 for records in points.values())
        ...
        meta = {"columns": columns, "rows": len(rows), "date_verified": day,
                ...
                "unpaired_points_by_ticker": unpaired,
