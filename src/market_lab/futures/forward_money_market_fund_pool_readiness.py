"""Replay-audit paired forward fund-pool snapshots and report sealed readiness."""
...
def assess(output_root: Path = source.DEFAULT_OUTPUT_ROOT) -> dict[str, Any]:
    ...
    return {
        ...
        "duplicate_stage_dates": {
            key: sorted(set(value)) for key, value in duplicates.items()
        },
        "fill_not_after_decision_dates": ordering_failures,
        "valid_pairs": valid_pairs,
        "fund_ranking_allowed": progress["current_phase"] != "discovery",
        "paper_economics_allowed": progress["current_phase"] == "unseen_evaluation",
        "progress": progress,
    }
