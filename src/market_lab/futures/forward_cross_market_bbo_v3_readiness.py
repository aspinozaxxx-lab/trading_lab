"""Report source-only readiness for cross-market V3 CNY-perpetual snapshots."""
...
from market_lab.futures import moex_forward_cross_market_bbo_source_v3 as source
...
def readiness(root: Path | None = None) -> dict[str, Any]:
    ...
    complete = [
        ...
        if manifest.get("status") == "complete_core_quotes"
    ]
    per_session = Counter(str(manifest["source_date"]) for manifest in complete)
    minimum = int(config["readiness"]["minimum_quote_complete_snapshots_per_session"])
