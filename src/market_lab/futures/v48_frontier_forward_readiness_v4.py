"""V48 source readiness admitting the sealed anonymous FRED transport V2."""
...
from market_lab.futures import v48_frontier_forward_readiness_v3 as parent
...
def load_config() -> dict[str, Any]:
    ...
    v48_v2.load_config()
    parent_protocol_sha256 = component_readiness._sha(
        component_readiness.PROJECT_ROOT
        / "configs/v48_frontier_forward_fred_api_correction_v1.yaml"
    )
    if (
        config["parent_readiness"]["protocol_sha256"]
        != parent_protocol_sha256
        or parent_config["protocol_id"] != "v48_frontier_forward_fred_api_correction_v1"
        or config["route_policy"]["valid_FRED_API_KEY"] != "authenticated_API_v1_only"
        or config["route_policy"]["absent_FRED_API_KEY"]
        != "anonymous_fredgraph_header_v2_only"
        or config["economic_invariants"]["V27_V39_V48_V49_signal_or_weight_changed"]
        is not False
        or config["economic_invariants"]["warmup_evaluation_or_promotion_gates_changed"]
        is not False
        or config["live_trading_allowed"] is not False
    ):
        raise ValueError("V48 FRED transport V2 readiness drifted")
    return config
...
def assess(
    ...
    return {
        ...
        "FRED_route_if_collected_now": (
            "authenticated_official_API"
            if key_configured
            else "anonymous_fredgraph_header_v2"
        ),
