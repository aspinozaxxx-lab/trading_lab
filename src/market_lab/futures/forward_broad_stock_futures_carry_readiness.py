"""Report source-only readiness for broad stock-futures carry snapshots."""
...
def readiness(root: Path | None = None) -> dict[str, Any]:
    ...
    for snapshot in snapshots:
        try:
            ...
            status_counts[str(manifest.get("status", "missing"))] += 1
    ...
    complete = [
        ...
        if manifest.get("status") == "complete_30_pairs_valid"
    ]
    by_session = Counter(str(manifest["source_date"]) for manifest in complete)
    minimum = int(config["readiness"]["minimum_complete_snapshots_per_session"])
    ...
    discovery_ready = len(complete_sessions) >= discovery_required
    return {
        "protocol_id": config["protocol_id"],
        "protocol_sha256": source.CONFIG_SHA256,
        "source_root": str(source_root),
        "source_only": True,
        "basis_rank_return_signal_trade_or_pnl_computed": False,
        "live_trading_allowed": False,
        "counts": {
            "snapshot_directories": len(snapshots),
            "audited_snapshots": len(valid_manifests),
            "complete_30_pair_snapshots": len(complete),
        ...
        "next_action": (
            "seal_broad_carry_economic_protocol_without_discovery_outcomes"
            if discovery_ready
            else "continue_immutable_broad_pair_collection"
        ),
...
def main() -> None:
    ...
    print(json.dumps(readiness(args.source_root), ensure_ascii=False, indent=2))
