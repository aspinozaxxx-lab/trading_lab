...
Следующий шаг требует разрешения пользователя на внешний запрос, не нового обучения.
Приоритет — узнать условия небольшого теста персонального доступа, а не покупать
полный корпоративный продукт или писать ещё один downloader. Черновик запроса
`algopack@moex.com` ниже **не отправлен**:
...
поставщика не подтвердят цель 20%/50% сами по себе. Пока сохраняется
