...
Update2026-09-07: [ответ MOEX прочитан](ALGOPACK_VENDOR_REPLY_20260907.md) через почту,
UI displayed12:11. Подтверждены личное использование и SYSTIME как publication time.
Историческая revision semantics в ответе не раскрыта; model/live gates не сняты.
FO OrderStats полный продукт только ожидается поставщиком до конца года, не доступен
по умолчанию. Новое письмо не отправлено; ниже сохраняется прежняя хронология.
