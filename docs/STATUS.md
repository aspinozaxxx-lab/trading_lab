...
## Текущее исследование — V67, ценовая премия конкретной ОФЗ после аукциона
...
не объявляются учтёнными. Только положительная ценовая зацепка переводится к полному
денежному расчёту. Local 6 synthetic tests PASS, Ruff PASS; economic run ещё не выполнен.
Метаданные/хеши существующих inputs сверены; новых collectors нет.
...
1. Завершить единственный sealed [V67](V67_OFZ_AUCTION_CONCESSION.md) механизм-screen,
   показать полный отсев/coverage/годовые event means и 1×/2× затраты. Не превращать
   средний пятидневный clean-price эффект в CAGR портфеля. Только survivor получает
   полный денежный расчёт. V65/V66: 8 отсеянных гипотез, не повторять/перенастраивать.
...
3. Не заполнять ожидание будущего paper старта минутными проверками таймера. Запуск
   AlgoPack оставить автономным; source sample индексных новостей пока не запускать.
