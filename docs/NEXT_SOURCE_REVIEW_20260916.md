...
## Текущий статус 17 сентября 10:15 МСК — V113 закрыт; equity scope / ETF flows

[V113](V113_PRIVATE_DEMAND_BREADTH_RESULT.md) отсеян на Stage1:
−8.8317% / −8.8898% CAGR, 13 roundtrips, один прибыльный год из пяти;
все четыре расчёта исполнены полностью. 43 портфельных теста, активных Stage2/3
нет. Не менять правило, не повторять расчёт, сбор или аудит без новой причины.

Задан отдельный вопрос пользователю: разрешён ли предварительный конкурс на уже
скачанных **equity TradeStats / OrderStats / OBStats 2020–2025**, с оговоркой об
условности нынешних исторических версий, без рынка 2026, демо и реальной торговли.
До явного ответа более широкий допуск не предполагать. При разрешении приоритет —
этому быстрому экономическому конкурсу; архив и исходный узкий futures screen
не равны такому разрешению.

Иначе следующий ограниченный обзор — **создание/погашение паёв физического золотого ETF**,
не цены золота и не переименование CFTC managed money. Предварительный поиск по
configs/docs/src нашёл только тикер UGLD в старых equity configs / filings,
не тест потоков GLD. Это не доказательство доходности или независимости.
Пока выполнен **только поиск по официальным сайтам**:

- [GLD issuer page](https://www.spdrgoldshares.com/usa/gld/) содержит ссылку на
  исторический XLSX, число паёв и массу золота; различает trade-date и settlement-date
  количества. Ни XLSX, ни полный численный корпус не открывались и не скачивались.
- [Historical archive information](https://www.spdrgoldshares.com/hong-kong/9840/english/historical-data/)
  указывает обновление на следующий день в 6:00–6:30 по Нью-Йорку, возможность
  изменений без уведомления и ограничение воспроизведения/распространения без
  письменного согласия. Это другой листинг; его время и права нельзя автоматически
  приписать точному американскому набору.
- [Issuer Japan page](https://www.spdrgoldshares.com/japan/gld/) тоже различает
  settlement bar list и trade-date website holdings. Эти величины нельзя подменять
  друг другом. Цены/NAV в поисковых фрагментах были AWAITING, численные рыночные
  исходы 2026 не читались. Ссылки на старые factsheets найдены, PDF не открывались.

Сначала проверить определения и права точного американского источника и допустимый
датированный образец не позже 2025 года. Не скачивать текущий XLSX с ценами/NAV
2026, не строить новый parser/framework и не объявлять часы листинга доказательством
первоначальной публикации. Если права или ограниченный образец недоступны — краткий
отказ и другая дешёвая гипотеза, без писем, покупок и обхода ограничений по умолчанию.
Правило, знак, актив, конфигурация и seal ещё не определены.

## Исторический статус 17 сентября 09:14 МСК — exporter FX отложен; рублёвые платежи
