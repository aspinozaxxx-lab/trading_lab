...
from market_lab.futures import algopack_futoi_archive_v2 as m
...
def test_sequences_keep_repeated_wall_clock():
    ...
    with pytest.raises(m.Error, match="daily_latest_group_not_unique"):
...
def test_bom_new_authored_files():
    for relative in (f"src/market_lab/futures/{m.PROTOCOL}.py",
                     ...
                     "docs/ALGOPACK_FUTOI_ARCHIVE_V2.md"):
        assert (Path(m.REPO) / relative).read_bytes().startswith(b"\xef\xbb\xbf")


def test_unpaired_source_rows_preserved_and_flagged():
    latest, groups = m.parse(payload(pair()[:1]), DAY)
    assert latest["rows"] == 1
    assert latest["unpaired_points_by_ticker"] == {"AA": 1}
    assert len(groups["AA"]) == 1
    rows = pair(seq=1)[:1] + pair(seq=2)[1:]
    parsed, groups = m.parse(payload(rows), DAY)
    assert parsed["unpaired_points_by_ticker"] == {"AA": 2}
    assert parsed["paired_points_by_ticker"] == {"AA": 0}
    assert {row["clgroup"] for row in groups["AA"]} == {"FIZ", "YUR"}
    intraday, _ = m.parse(payload(rows), DAY, "AA")
    assert intraday["latest_stable_sha256"] == parsed["latest_stable_sha256"]
