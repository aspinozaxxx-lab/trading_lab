"""Tests for the sealed weekly MOEX option-state source V3."""

from __future__ import annotations

import json
from collections.abc import Mapping
from datetime import date
from pathlib import Path
from urllib.parse import parse_qs, urlparse

import pandas as pd

from market_lab.futures import moex_options_surface_history_v2 as transport
from market_lab.futures import moex_options_weekly_state_source_v3 as source


def _payload(query_date: str) -> bytes:
    columns = source.load_config()["source"]["required_columns"]
    rows = []
    for security_id, strike in (("SI75000BA1", 10.0), ("SI80000BM1", 11.0)):
        row = {column: None for column in columns}
        row.update(
            {
                "TRADEDATE": query_date,
                "BOARDID": "ROPD",
                "SECID": security_id,
                "CLOSE": strike,
                "SETTLEPRICE": strike,
                "VOLUME": 1,
                "OPENPOSITION": 2,
                "NUMTRADES": 1,
            }
        )
        rows.append([row[column] for column in columns])
    return json.dumps(
        {
            "history": {"columns": columns, "data": rows},
            "history.cursor": {
                "columns": ["INDEX", "TOTAL", "PAGESIZE"],
                "data": [[0, 2, 100]],
            },
        }
    ).encode()


class _Response:
    def __init__(self, content: bytes) -> None:
        self.content = content

    def raise_for_status(self) -> None:
        return None


class _Session:
    def get(self, url: str, *, headers: Mapping[str, str], timeout: float) -> _Response:
        assert headers["User-Agent"] == transport.USER_AGENT
        assert timeout == 30.0
        query = parse_qs(urlparse(url).query)
        assert query["assetcode"] == ["Si"]
        assert query["start"] == ["0"]
        return _Response(_payload(query["date"][0]))


def test_config_and_calendar_are_exact_and_source_only() -> None:
    config = source.load_config()
    dates = source.decision_dates(config)

    assert config["objective"]["economic_rule_fixed_by_this_protocol"] is False
    assert len(dates) == 261
    assert dates[0] == date(2021, 1, 8)
    assert dates[-1] == date(2025, 12, 30)
    assert len(source.canonical_jobs(config)) == 1044


def test_small_source_collects_and_raw_replays(tmp_path: Path) -> None:
    output = source.collect(
        tmp_path / "options",
        jobs=[(date(2021, 1, 8), "Si")],
        session=_Session(),
        retrieved_at="2026-09-02T02:40:00Z",
    )
    frame = pd.read_parquet(output / "options_weekly_core4.parquet")
    manifest = json.loads((output / "manifest.json").read_text(encoding="utf-8-sig"))

    assert len(frame) == 2
    assert set(frame["logical_asset"]) == {"SI"}
    assert frame["available_at_utc"].eq(pd.Timestamp("2021-01-08T21:00:00Z")).all()
    assert manifest["contains_returns_targets_predictions_or_pnl"] is False
    assert all(source.audit(output).values())
